...
- Debug UI-test workspaces can now expose any requested left/right and top/bottom safe edge of the unchanged production-minimum window on constrained displays. Generic inline-text coverage uses Command-Return, while dedicated journeys retain real pointer clicks for status Commit/Cancel and toolbar Preview/Undo/Redo; Release sizing and placement are unchanged.
