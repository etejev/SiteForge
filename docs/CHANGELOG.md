...
- Debug UI-test workspaces can now expose requested left/right/top safe edges and fit only a bottom-aligned test frame when the titled production-minimum frame is taller than the usable display. Generic inline-text coverage uses Command-Return, while dedicated journeys retain real pointer clicks for status Commit/Cancel and toolbar Preview/Undo/Redo; the production 1100×700 content minimum and Release sizing/placement are unchanged.
