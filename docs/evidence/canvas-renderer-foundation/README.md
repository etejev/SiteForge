...
This directory retains production-representative evidence for the bounded `SF-0407-001` through `SF-0407-008` renderer foundation. It does not claim canonical render-pass editing or persistence, export generation, production text shaping/font fallback, image/effect rasterization, stress-limit acceptance, VoiceOver speech, or release performance. A bounded committed plain-text tile path is separately covered below.
...
- Tile drawing covers bounded placeholder rectangles, clipping, appearance-derived colors, focus, and bounded committed plain-text rasterization. Production shaping, font fallback, images, effects, and resource decoding remain later work.
