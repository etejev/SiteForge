...
| Hosted compact-window accessibility | Hosted run `32899615186` reached the complete UI target and exposed three display-height/identity assumptions: gradient-stop wells were required to be simultaneously visible instead of reachable through the real Inspector scroll surface; the flexible Inspector tab strip could consume the overflow menu's pointer width; and SwiftUI could reuse the Assets unavailable subtree after selecting Components. The tab strip now reserves the native overflow target, each future destination owns a distinct accessibility subtree, and the ordered-fill journey scrolls each real well into view before retaining its pointer contract. | Responsive native panes must allocate fixed trailing actions before flexible scroll content. Reused unavailable-state views require stable destination identity, and UI acceptance must prove that deep controls are reachable through the shipping scroll surface rather than assume a particular hosted display height. | The corrected app and UI targets compile; `AppMetadataTests` passed 14/14 and repository checks passed. A focused local UI launch was blocked before test bodies by the separately recorded macOS automation foreground denial; hosted Actions is the authoritative rerun for these three exact journeys. |
